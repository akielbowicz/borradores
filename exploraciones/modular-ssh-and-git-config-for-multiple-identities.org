...
*Important*: The glob pattern must be =*/**= not just =**=. The =**= pattern requires a =/= path separator to match, but SSH URLs have the format =git@host:Org/repo.git= (no slash after the colon). Using =*/**= correctly matches =Org/repo.git=.
...
- *No per-directory .gitconfig files needed*: the URL-based matching eliminates the need for .gitconfig files in each workspace directory
